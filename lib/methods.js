var ensureTeacher = function(room) {
	if(!Meteor.userId()) {
		throw Meteor.Error(403, "Access Denied");
	}
	if(room.teachers.indexOf(Meteor.userId()) !== -1) {
		return;
	}
...
Meteor.methods({
	...
	createUser2: function(username, password, readSubjects, writeSubjects, userId, locked) {
		if(Meteor.user() && Meteor.user().roles.indexOf("admin") !== -1) {
			if(!userId) {
                ...
                if(locked) {
                  Meteor.users.update({_id: userId, roles: { $nin: ["admin"] }}, {$set: {username: username, locked: true}});
                } else {
                  Meteor.users.update(userId, {$set: {username: username, locked: false}});
                }
			}
	...
	createFolder: function(roomId, active, pathD, folderName) {
		...
		var room = Rooms.findOne(roomId, {fields: {files: 1, quizzes: 1, teachers: 1}});
		ensureTeacher(room);
		if(!room[active]) {
			room[active] = [];
		}
		...
		if(path === pathD) {
			if(_.findWhere(b, {name:folderName})) {
				throw new Meteor.Error("Path already exists.");
			}
	...
	addLink: function(roomId, path, data) {
		...
		x.push({type: "link", name: data.name, url: data.url});
		var c = {files: room.files};
		Rooms.update(roomId, {$set: c});
	},

	rename: function(roomId, active, path, old, folderName) {
		if(["files", "quizzes"].indexOf(active) === -1) {
			return;
		}
		var room = Rooms.findOne(roomId);
	...
	del: function(roomId, active, path, name) {
		if(["files", "quizzes"].indexOf(active) === -1) {
			return;
		}
		var room = Rooms.findOne(roomId);
	...
	createQuiz: function(roomId, path, title, questions) {
		var room = Rooms.findOne(roomId, {fields: {quizzes: 1, teachers: 1}});
		ensureTeacher(room);
		var x = room.quizzes;
		if(path !== "/") {
			path.split("/").forEach(function(p) {
	...
	attemptQuiz: function(quizId, questions) {
		...
		quiz.questions.forEach(function(y, i) {
			...
			if(y.type ==="fill_in_the_blanks") {
					y.marks = 0;
					y.fitbAnswer = questions[i].fitbAnswer;
				...
				if(y.marks === y.fitbAnswer.length) {
					y.correct = true;
				}
			} else {
				c.max++;
	...
	reply: function(roomId, noticeId, type, text) {
		var comment = {comments:{userId: Meteor.userId(), text: text}};
		if(type === "notice") {
	...
	delComment: function(noticeId, type, commentId, roomId) {
		var query = {_id: noticeId, roomId: roomId};
		if(Meteor.user()) {
			console.log(Meteor.user());
			console.log(roomId);
			console.log(Rooms.findOne(roomId));
			if(Meteor.user().roles.indexOf("admin") === -1 && Rooms.findOne(roomId).teachers.indexOf(Meteor.userId()) === -1) {
				if(type === "notice") {
					Notices.update(query, {$pull: {comments: {userId: Meteor.userId(), commentId: commentId}}});
				} else if(type === "poll") {
					Polls.update(query, {$pull: {comments: {userId: Meteor.userId(), commentId: commentId}}});
				} else if(type === "reminder") {
					Reminders.update(query, {$pull: {comments: {userId: Meteor.userId(), commentId: commentId}}});
				} else if(type === "assignment") {
					Assignments.update(query, {$pull: {comments: {userId: Meteor.userId(), commentId: commentId}}});
				}
			} else {
				console.log(query);
				console.log(commentId);
				if(type === "notice") {
					Notices.update(noticeId, {$pull: {comments: {commentId: commentId}}});
				} else if(type === "poll") {
					Polls.update(noticeId, {$pull: {comments: {commentId: commentId}}});
				} else if(type === "reminder") {
					Reminders.update(noticeId, {$pull: {comments: {commentId: commentId}}});
				} else if(type === "assignment") {
					Assignments.update(noticeId, {$pull: {comments: {commentId: commentId}}});
				}
			}
		}
	},
	...
	join: function(id) {
		Rooms.update({_id: id, type: "club"}, {$push: {students: Meteor.userId()}});
	},

	unjoin: function(id) {
		Rooms.update({_id: id, type: "club"}, {$pull: {students: Meteor.userId()}});
	}
});
