Meteor.methods({
  ...
  "vocabularyQuiz": function(courseId, data, vocabQuizId) {
    ...
    "quiz": function(courseId, data, quizId) {
      if(isTeacher(courseId)) {
        if(quizId) {
          Courses.update({_id:courseId, "quizzes._id":quizId}, {$set: {"quizzes.$.title": data.title, "quizzes.$.questions":data.questions}});
        } else {
  ...
  "fileComment": function(fileId, grade, comment) {
    Files.update(fileId, {$set: {grade: grade, comment: comment}});
  }
});
